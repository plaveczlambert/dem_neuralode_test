...
class VanDerPol(nn.Module):
    '''The ODE function of Van der Pol equation'''
...
with torch.no_grad():
    true_y = odeint(VanDerPol(), true_y0, t, method='dopri5', rtol=1e-10, atol=1e-10)
...
if __name__ == '__main__':
    ...
    iterations = np.linspace(start_iter, start_iter+args.niters, args.niters)
    ...
    if not args.early_stop:
        torch.save({
            'iteration': start_iter+args.niters,
            'model_state_dict': model.state_dict(),
            #'scheduler_state_dict': scheduler.state_dict(),
            'optimizer_state_dict': optimizer.state_dict()
            },
            args.save_path+'model_' + (args.name+'_' if args.name else '') + 'i' + str(start_iter+args.niters) + '_' + time_str + '.pt')
