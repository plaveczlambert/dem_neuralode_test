...
from utils.scalers import writeStandardScaler, writeMinMaxScaler
...
parser.add_argument(
    ...
    default=0, 
    type=int, 
    help= "Early stop when no improvement in validation loss is achieved for the given number of epochs"
    )
parser.add_argument(
    '--num_threads',
    default = 0,
...
parser.add_argument(
    '--data',
    default = os.path.join('data', 'keller_data.hdf5'),
    type    = str,
...
# Data scaling
# ----- ----- ----- ----- ----- -----
out_scaler  = QuantileTransformer(output_distribution="normal", random_state=42, copy=False)
in_scaler = MinMaxScaler(feature_range=(0, 1), copy=False)
...
loss    = nn.MSELoss()
optim   = torch.optim.Adam(model.parameters(), lr=args.lr, eps=1e-8)
#base_lr = 1e-4
#max_lr = 5e-5
#scheduler = torch.optim.lr_scheduler.OneCycleLR(optim, max_lr=0.01, steps_per_epoch=)
if args.load_model:
...
for num_epoch in range(args.epoch):
    ...
    if vld_loss < vld_loss_best:
        ...
        if args.early_stop and num_epoch-best_epoch == args.early_stop:
            if not args.test:
...
if not args.test:
    #save scalers
    scaler_path = args.save_path+'scaler_' + (args.name+'_' if args.name else '') +time_str + '.psca'
    f = open(scaler_path,'w')
    
    if type(out_scaler) == StandardScaler:
        writeStandardScaler(f, out_scaler)
    elif type(out_scaler) == MinMaxScaler:
        writeMinMaxScaler(f, out_scaler)
    else:
        import pickle
        pickle_path = args.save_path+'out_scaler_' + (args.name+'_' if args.name else '') +time_str + '.psca.pickle'
    ...
    torch.save({
        ...
        args.save_path+'model_' + (args.name+'_' if args.name else '') + 'e' + str(start_epoch+best_epoch) + '_' + time_str + '.pt')
    ...
    traced_model_path = args.save_path+'traced_model_' + (args.name+'_' if args.name else '') + 'e'+str(start_epoch+best_epoch) + '_' + time_str + '.pt'
...
if args.monitor>0:
        plt.show()
if not args.test:
    plt.savefig(args.save_path+"Loss_distr_"+time_str+".pdf")
...
if not args.test:
    plt.savefig(args.save_path+"Loss_distr_full_"+time_str+".pdf")
...
if not args.test:
    ...
    dem_sol = scipy.integrate.solve_ivp(keller.ode, [0, 3], [ 1.0, 0.0], method=DeepEuler, h=1e-4,
    ...
    plt.plot(eul_sol.t,eul_sol.y[1,:], color="silver", label="Euler 1e-4")
    plt.plot(dem_sol.t,dem_sol.y[1,:], color="purple", label="DEM 1e-4", linestyle="--", dashes=(5,5))
    #plt.plot(ad_dem_sol.t,ad_dem_sol.y[0,:], linestyle="--", dashes=(5,5), color="orange", label="ADEM")
    plt.ylabel("$x_2$")
    plt.xlabel("$t$")
    plt.xlim([1.5, 3])
    ...
    plt.show()
    if not args.test:
        plt.savefig(args.save_path+"IntegrationComparison_"+time_str+".pdf")
