...
class DeepEuler(OdeSolver):
    ...
    def __init__(self, fun, t0, y0, t_bound, h, traced_model, in_scaler=None, out_scaler=None, mode=MODE_GENERAL, **extraneous):
    ...
    def _step_impl(self):
        ...
        if self.in_scaler:
            y_in = self.in_scaler.transform(y_in.reshape(1,-1))
            
        
        if self.out_scaler:
            model_out = self.out_scaler.inverse_transform(self.model(torch.tensor(y_in)).detach().numpy())
        else:
            model_out = self.model(torch.tensor(y_in)).detach().numpy()
        
        y_new = y + h * dydt + h * h * model_out.reshape(2)
        t_new = t + h
